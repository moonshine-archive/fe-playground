...
const Triangle: React.FC = () => {
  ...
  const requestRef = useRef<number>(0) // 애니메이션 프레임 ID를 저장하기 위한 ref
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      // 애니메이션 시작
      requestRef.current = requestAnimationFrame(() => drawScene(gl, programInfo, buffers))
  ...
  const drawScene = (gl: WebGLRenderingContext, programInfo: ProgramInfo, buffers: Buffers) => {
    ...
    // 애니메이션 루프를 계속하기 위해 requestAnimationFrame을 다시 호출
    requestRef.current = requestAnimationFrame(() => drawScene(gl, programInfo, buffers))
  }
